        ...
        <div class="col-xl-9 col-lg-9 col-md-9 col-sm-10 float-none p-0 mx-auto item-summary wallet-wrapper">
            ...
            <div class="variable-width mb-3">
                @foreach($cards as $card)
                    ...
                    @endphp
                    <div class="item credit-card-{{$css}} out-{{$card->Token}}" data-mh="matchHeight"
                         id="out-{{$card->Token}}">
                        <div class="item-div text-white p-3 in-{{$card->Token}}" id="in-{{$card->Token}}">
                            <a href="javascript:void(0)" onclick="DeleteCards({{$card->Id}})"
                               class="d-inline-block"><img
                                    src="{{asset('assets/images/icon-checkout-close.png')}}"/></a>

                            <div class="card-code">{{$card->Card}}</div>
                            <div class="clearfix"></div>
                            <div class="card-date">Valid Thru<br>{{$month.' / '.$year}}</div>
                            <div class="buttons text-center mt-3">
                                <a href="javascript:void(0)" onclick="SelectCard('{{$card->Token}}')"
                                   class="btn btn-redeem active text-uppercase">@lang('pay_with_this_card')</a>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="action-buttons text-center pt-4">
                ...
                <button type="button"
                        class="btn btn-B3B3B3 text-uppercase new-card">@lang('pay_with_a_new_card')</button>
            </div>
        </div>
    </div>
    <div class="OrderSummary modal fade" id="OrderSummary" tabindex="-1" role="dialog"
