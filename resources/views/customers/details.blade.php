            ...
            <div class="loyaltygraph-wrapper accordion" id="orderexpanded">
                <div class="row profile-btn">
                    ...
                    <div class="col-md-9">
                        <button type="button" id="edit-profile"
                                class="btn btn-8DBF43 btn-1-anim btn-user">@lang('edit_profile')</button>&nbsp;&nbsp;
                        <a href="{{route('logout')}}"
                           class="btn btn-808080 btn-1-anim btn-user">&nbsp;&nbsp;<?php echo app('translator')->get('logout'); ?>
                            &nbsp;&nbsp;</a>
                    </div>
                </div>
                ...
                @if(isset($LEVEL_ID) and $LEVEL_ID!='')
                    <div style="margin-top: 50px;"
                         class="col-xl-8 col-lg-10 float-none p-0 mx-auto loyaltygraph-div pt-4 mb-5 pb-3">
                        <div class="row">
                            @php
                            ...
                            <div class="col-md-6">
                                <div class="c100 p{{$per}} big green">
                                    <img src="/assets/images/arrow-down.png"
                                         class="{{($query->details->Threshold> $query->details->TierBalance) ? '':'d-none'}}"
                                         alt="zwz profile arrow">
                                    <span><div
                                            style="font-size: 68px !important;">{{$query->details->LevelName}} <br> <small>{{number_format($query->details->TierBalance)}} @lang('points_lower')</small></div></span>
                                    <div class="slice">
                                        <div class="bar"></div>
                                        <div class="fill"></div>
                                    </div>
                            ...
                            <div class="col-md-3 d-flex align-items-end">
                                <div style="font-size: 16px !important;">
                                    @if($query->details->Threshold> $query->details->TierBalance)
                                        COLLECT {{$query->details->Threshold-$query->details->TierBalance}} points for
                                        a {{$query->details->TotalOp}} % discount
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-xl-11 col-lg-10 float-none p-0 mx-auto wallet-wrapper">
                        ...
                        <div id="wallet-carousel" class="owl-carousel wallet-carousel">
                                ...
                                <div class="slide-shadow item active bg-8DBF43" data-mh="matchHeight" id="wallet-b">
                                    <div class="item-div active text-white p-3" id="wallet-b-1">
                                        ...
                                        <div class="item-discount text-uppercase">
                                            @lang('you_have') <span
                                                class="wallet-balance">{{number_format($wallet_balance)}}</span> {{$currency}}
                                        </div>

                                        <p><img src="{{asset('assets/images/icon-logowhite.png')}}"
                                                class="w-auto logo-img"></p>
                                    </div>
                            ...
                            @for($i=0;$i<count($vouchers);$i++)
                                ...
                                @endphp
                                <div class="slide-shadow item " style="background-color: {{$bg_color}}"
                                     id="voucher-b{{$vouchers[$i]->Id}}">
                                    <div class="item-div text-white p-3" id="voucher-b1{{$vouchers[$i]->Id}}">
                                        <div class="py-4 item-quantity text-right float-right">
                                            <div class="float-right"><span class="qty{{$vouchers[$i]->Id}}"
                                                                           data-title="{{count($vouchers[$i]->Vouchers)}}">{{count($vouchers[$i]->Vouchers)}}</span> @lang('quantity')
                                            </div>
                                        </div>
                                        <div class="item-discount text-uppercase title-{{$vouchers[$i]->Id}}"
                                             data-title="{{$vouchers[$i]->Title}}">
                                            {{$vouchers[$i]->Title}}
                                        </div>
                                        @php
                                        ...
                                        <div class="items-vouchers">
                                            ...
                                            @foreach($array_exp as $key=>$value)
                                                <div class="voucher"><span class="{{$css}}"
                                                                           data-title="{{$value}}">{{$value}}</span> @lang('vouchers_expire') {{$key}}
                                                </div>
                                                @php $css='' @endphp
                                            @endforeach
