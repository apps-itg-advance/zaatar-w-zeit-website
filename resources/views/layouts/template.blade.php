...
<div id="app" class="content-wrapper {{$class_css ?? 'favourites-wrapper' }} py-5">
    <div class="container">
        <div class="content-container">
            <div class="row">
                @php
                    ...
                    $org = session()->has('_org') ? session()->get('_org') : (object) [];
                    $user = session()->has('user' . $Skey) ? session()->get('user' . $Skey) : (object) [];
                    $addresses = session()->has('addresses' . $Skey) ? session()->get('addresses' . $Skey) : [];
                @endphp
                <div style="display:none">
                    <main-component :org="{{json_encode($org)}}" :user="{{json_encode($user)}}"
