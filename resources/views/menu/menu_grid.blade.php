            ...
            <div class="col-favourite">
                <div class="favourite-box">
                    <div class="media media-item">
                        ...
                        <img alt="loading.."
                             data-src="{{asset(isset($row->LocalThumbnailImg) ? $row->LocalThumbnailImg : $row->ThumbnailImg)}}"
                             @if($has_modifier==1) style="cursor: pointer" onclick="OpenModel({{$row->ID}})"
                             @endif class="mr-3 img-thum b-lazy" alt="...">
                        <div class="media-body">
                            <h5 class="mt-0">
                                <a id="ItemName{{$row->ID}}" href="javascript:void(0)"
                                   @if($has_modifier==1) onclick="OpenModel({{$row->ID}})"
                                   @endif style="max-width: 60% !important; float: left">{{$item_name}}</a>
                                <span class="price"
                                      style="max-width: 38% !important; float:right; vertical-align: text-top">{{number_format($row->Price)}} {{$currency}}</span>
                                <div class="clearfix"></div>
                                <ul class="icon">
                                    <?php
                                    foreach ($array_icon as $key => $val) {
                                        if (isset($row->$key) and $row->$key == "1") {
                                            echo '<li><img src="' . $val['URL'] . '" /></li>';
                                        }
                                    }
                                    ?>
                                </ul>
                    ...
                    <div class="mediabox row align-items-center">
                        <div class="col-sm-7 text-center">
                            ...
                            @if(!$display_favourite)
                                @if(session('is_login'))
                                    <a onclick="SetFavourite({{$row->ID}})"
                                       ...
                                       id="Favourite{{$row->ID}}" href="javascript:void(0)"
                                       class="favUnfav{{$row->ID}} effect-underline link-favourite mr-3 {{$active_f}}"><span>@lang('favourite')</span></a>
                                @else
                                    <a onclick="loginAlert()"
                                       class="effect-underline link-favourite mr-3 cursor-pointer"><span>@lang('favourite')</span></a>
                                @endif
                            @endif
                            @php
                                $x=0;
                            ...
                            @if($has_modifier==1)
                                ...
                                <a onclick="OpenModel({{$row->ID}})" id="CustomizedLink{{$row->ID}}"
                                   class="link-customize pointer effect-underline {{$cust_css}}"><span
                                        class="customize-label"
                                        id="Customize{{$row->ID}}">@lang('customized')</span></a>
                            @endif
                        </div>
                            ...
                            <div class="input-group mx-auto item-plus-minus">
                                <div class="input-group-append">
                                    <button type="button" class="btn btn-link btn-link-minus pointer"
                                            data-code="{{$row->ID}}" onclick="SubQty({{$row->ID}},{{$row->PLU}})">&nbsp;
                                    </button>
                                </div>
                                <input type="text" name="qty[{{$row->ID}}]" id="qty_{{$row->ID}}"
                                       class="form-control qty_all"
                                       value="{{isset($item_qty[$row->PLU])? $item_qty[$row->PLU]:0}}"
                                       style="background: none !important" readonly="readonly">
                                @if($row->QuickOrder==1)
                                    <div class="input-group-prepend">
                                        <button type="button" class="btn btn-link btn-link-plus pointer"
                                                data-code="{{$row->ID}}" onclick="AddQty({{$row->ID}})">&nbsp;
                                        </button>
                                    </div>
                                @endif
                            </div>
                    ...
                    @if($display_favourite)
                        ...
                        <a href="#!" class="link-close remove-fav-item" data-code="{{$row->FavouriteId}}"><img
                                src="{{asset('assets/images/icon-close.png')}}"/></a>
                    @endif
                </div>
            </div>
